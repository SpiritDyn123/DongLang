...
DongLangTypeInfo* SLSymbol::typeCheckTrans(DongLangTypeInfo* t1, DongLangTypeInfo* t2, string opr, bool errReport, string reportStr) {
	if (!t1 || !t2) {
		return NULL;
	}
	...
	if (errReport) {
		DongLangBaseAST::llvmCtx->emitError(reportStr + " opr:"+  opr + " type trans error:" + t2S + "=>" + t1S);
	}
...
string FuncSLSymbol::funcID(string baseId, vector<DongLangTypeInfo*> types, bool isVarArg) {
	...
	if (baseId != "main" && baseId != "global_main_init") {
		...
		id = regex_replace(id, regex("\\*"), "PTR");
		id = regex_replace(id, regex("\\["), "ARR");
		id = regex_replace(id, regex("\\]"), "RRA");
	}
